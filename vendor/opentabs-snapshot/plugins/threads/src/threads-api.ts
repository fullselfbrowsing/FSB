// Hermetic transport-helper stub for the vendored threads metadata slice.
//
// Upstream threads-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Threads web app on www.threads.net. The importer NEVER executes a handle() body
...
// facts (preserved here as comments for auditability): Threads reads the timeline +
// a single thread via `api` (default GET); creating a thread POSTs a new post (the
// sensitive social WRITE -- `create` is in the shared WRITE_VERBS set and the
// {method:'POST'} literal reinforces the write class on both the verb AND method axes).
...
const inert = (helper: string): never => {
  throw new Error(
    `threads-api stub: ${helper} is metadata-only and must never execute at ` +
