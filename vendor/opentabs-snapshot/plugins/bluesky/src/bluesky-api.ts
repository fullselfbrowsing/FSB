// Hermetic transport-helper stub for the vendored bluesky metadata slice.
//
// Upstream bluesky-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Bluesky web app on bsky.app (the AT Protocol XRPC surface). The importer NEVER
...
// facts (preserved here as comments for auditability): Bluesky reads the timeline +
// a profile via `api` (default GET); creating a post POSTs a new record
// (create_post -> the sensitive social WRITE; `create` is in the shared WRITE_VERBS
// set and the {method:'POST'} literal reinforces write); deleting a post is the
// DESTRUCTIVE op (delete_post -> apiVoid {method:'DELETE'} -> apiDelete/destructive).
...
const inert = (helper: string): never => {
  throw new Error(
    `bluesky-api stub: ${helper} is metadata-only and must never execute at ` +
