...
export const searchHotels = defineTool({
  ...
  description:
    'Search KAYAK across providers for hotels in a destination for a set of dates and number of guests. Returns matching nightly prices from multiple agencies.',
  summary: 'find hotels on kayak',
  icon: 'building',
  group: 'Hotels',
  input: z.object({
  ...
  output: z.object({
    hotels: z.array(z.object({
      ...
      name: z.string(),
      price: z.number(),
    })).describe('Matching nightly prices across providers'),
  }),
  handle: async (params: { destination: string; check_in: string; check_out: string; guests?: number }) => {
    ...
    return { hotels: data.hotels as { id: string; name: string; price: number }[] };
  },
});
