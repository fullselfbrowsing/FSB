// Hermetic transport-helper stub for the vendored instacart metadata slice.
//
// Upstream instacart-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Instacart web app on www.instacart.com. The importer NEVER executes a handle()
...
// facts (preserved here as comments for auditability): Instacart lists stores +
// searches products + lists the user's orders via `api` (default GET, reads);
// CHECKING OUT a cart POSTs it (checkout -> the PAYMENT WRITE; the {method:'POST'}
// literal reinforces write on both axes -- a checked-out cart is money moved);
// CANCELLING an order is the DESTRUCTIVE op (cancel_order -> apiVoid
// {method:'DELETE'} -> apiDelete/destructive). www.instacart.com is SENSITIVE (39-01)
...
const inert = (helper: string): never => {
  throw new Error(
    `instacart-api stub: ${helper} is metadata-only and must never execute at ` +
