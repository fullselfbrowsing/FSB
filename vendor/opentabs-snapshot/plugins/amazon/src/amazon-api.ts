// Hermetic transport-helper stub for the vendored amazon metadata slice.
//
// Upstream amazon-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Amazon web app on www.amazon.com. The importer NEVER executes a handle() body
...
// facts (preserved here as comments for auditability): Amazon searches the product
// catalog + a single product detail + the user's orders + a single order's tracking
// via `api` (default GET, reads); PLACING an order POSTs the cart (place_order ->
// the PAYMENT WRITE; the {method:'POST'} literal reinforces write on both axes -- a
// paid order charges the saved card); CANCELLING an order is the DESTRUCTIVE op
// (cancel_order -> apiVoid {method:'DELETE'} -> apiDelete/destructive).
// www.amazon.com is SENSITIVE (39-01) -> posture-B re-gates the writes, and
...
const inert = (helper: string): never => {
  throw new Error(
    `amazon-api stub: ${helper} is metadata-only and must never execute at ` +
