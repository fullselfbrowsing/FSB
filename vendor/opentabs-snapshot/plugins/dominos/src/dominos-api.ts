// Hermetic transport-helper stub for the vendored dominos metadata slice.
//
// Upstream dominos-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Domino's web app on www.dominos.com. The importer NEVER executes a handle() body
...
// (preserved here as comments for auditability): Domino's lists nearby stores + a
// store's menu + the user's orders + a single order's live tracking via `api` (default
// GET, reads); PLACING an order POSTs the cart (place_order -> the PAYMENT WRITE; the
// {method:'POST'} literal classes it write -- 'place' is NOT a side-effect WRITE_VERB,
// so the POST is what classes it write, AND 'place' is in 39-01 PAYMENT_VERBS +
// place_order in PAYMENT_OP_NAMES -- a placed order charges the saved card).
// www.dominos.com is SENSITIVE (39-01) -> posture-B re-gates the writes, and
// backing:'dom' keeps place_order DOM-only (NOT API-invocable -> the payment-op guard passes).
...
const inert = (helper: string): never => {
  throw new Error(
    `dominos-api stub: ${helper} is metadata-only and must never execute at ` +
