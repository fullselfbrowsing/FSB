// Hermetic transport-helper stub for the vendored clickup metadata slice.
//
// Upstream clickup-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// ClickUp REST API v2. The importer NEVER executes a handle() body (Wall 1), but
// the tool modules reference `api` / `apiVoid` in their (never-run) handle
...
// facts (preserved here as comments for auditability): ClickUp is a REST app --
// `api` defaults to GET (reads) and is called with {method:'POST'} for create and
// {method:'PUT'} for update; `apiVoid` is the 204 mutating helper (default POST,
// {method:'DELETE'} for a destructive op).
...
const inert = (helper: string): never => {
  throw new Error(
    `clickup-api stub: ${helper} is metadata-only and must never execute at ` +
