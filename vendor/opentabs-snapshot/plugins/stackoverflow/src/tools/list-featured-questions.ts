...
export const listFeaturedQuestions = defineTool({
  name: 'list_featured_questions',
  displayName: 'List Featured Questions',
  description:
    'List Stack Overflow questions that currently have an active bounty. These are questions where users have offered reputation as a reward for a good answer.',
  summary: 'List questions with active bounties',
  icon: 'star',
  group: 'Questions',
  input: z.object({
  ...
  output: z.object({
    questions: z.array(questionSchema).describe('Featured questions with active bounties'),
  ...
  handle: async params => {
    const data = await api('/questions/featured', {
      query: {
