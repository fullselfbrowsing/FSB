...
export const getQuestionComments = defineTool({
  name: 'get_question_comments',
  displayName: 'Get Question Comments',
  description:
    'Get comments on a specific Stack Overflow question. Returns comment body, score, and author information.',
  summary: 'Get comments on a question',
  icon: 'message-circle',
  group: 'Questions',
  input: z.object({
  ...
  output: z.object({
    comments: z.array(commentSchema).describe('Comments on the question'),
  ...
  handle: async params => {
    const data = await api(`/questions/${params.question_id}/comments`, {
      query: {
