...
import { z } from 'zod';
import { apiVoid } from '../shopify-api.js';
...
export const cancelOrder = defineTool({
  ...
  summary: 'cancel my shopify order',
  icon: 'x-circle',
  group: 'Orders',
  input: z.object({
  ...
  handle: async (params: { order_id: string; reason?: string }) => {
    // NEVER executed by the importer. Upstream: apiVoid DELETE /admin/api/orders/:id
    // (cancel -> DESTRUCTIVE via the shared verb set; apiVoid {method:'DELETE'} -> apiDelete/destructive).
    await apiVoid(`/admin/api/orders/${params.order_id}`, { method: 'DELETE', body: { reason: params.reason } });
    return { success: true };
  },
});
