// Hermetic transport-helper stub for the vendored airbnb metadata slice.
//
// Upstream airbnb-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Airbnb web app on www.airbnb.com. The importer NEVER executes a handle()
...
// facts (preserved here as comments for auditability): Airbnb searches listings + a
// listing's detail + the user's trips via `api` (default GET, reads); BOOKING a stay
// POSTs the reservation (book_stay -> the PAYMENT WRITE; the {method:'POST'} literal
// reinforces write on both axes -- a booked stay charges a card); CANCELLING a
// reservation is the DESTRUCTIVE op (cancel_reservation -> apiVoid {method:'DELETE'}
// -> apiDelete/destructive). www.airbnb.com is SENSITIVE (39-01) -> posture-B
// re-gates the writes, and backing:'dom' keeps book_stay DOM-only (NOT API-invocable
// -> the payment-op guard passes).
...
const inert = (helper: string): never => {
  throw new Error(
    `airbnb-api stub: ${helper} is metadata-only and must never execute at ` +
