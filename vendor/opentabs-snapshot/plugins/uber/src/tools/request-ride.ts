...
export const requestRide = defineTool({
  ...
  description:
    'Request a paid Uber ride: book a selected ride option from a pickup to a dropoff. This charges your saved payment method and dispatches a driver -- a real money-moving action.',
  summary: 'request an uber ride',
  icon: 'navigation',
  group: 'Rides',
  input: z.object({
    pickup: z.string().min(1).describe('Pickup location or address'),
    dropoff: z.string().min(1).describe('Dropoff location or address'),
    ride_option_id: z.string().min(1).describe('The ride option to book'),
  ...
  output: z.object({
    trip: z.object({
      ...
      status: z.string(),
      fare: z.number(),
    }).describe('The requested trip'),
  }),
  handle: async (params: { pickup: string; dropoff: string; ride_option_id: string; payment_method_id?: string }) => {
    ...
    const data = await api<{ trip: { id: string; status: string; fare: number } }>('/v1/rides', {
      method: 'POST',
      body: {
        pickup: params.pickup,
        dropoff: params.dropoff,
        ride_option_id: params.ride_option_id,
