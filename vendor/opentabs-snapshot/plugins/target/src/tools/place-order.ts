...
export const placeOrder = defineTool({
  ...
  description:
    'Place a paid Target order: submit a cart of products for shipping or in-store pickup. This charges your saved payment method and commits the order -- a real money-moving action.',
  summary: 'order this on target',
  icon: 'shopping-cart',
  group: 'Orders',
  input: z.object({
    items: z.array(z.object({
      product_id: z.string().describe('Product TCIN/ID'),
      quantity: z.number().int().min(1).describe('Quantity of this item'),
    })).min(1).describe('The cart items to order'),
    fulfillment: z.enum(['ship', 'pickup', 'drive_up']).describe('Ship, in-store pickup, or Drive Up'),
