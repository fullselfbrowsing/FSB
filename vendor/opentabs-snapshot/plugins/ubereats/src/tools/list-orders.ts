...
export const listOrders = defineTool({
  ...
  summary: 'show me my ubereats order history',
  icon: 'receipt',
  group: 'Orders',
  input: z.object({
  ...
  handle: async (params: { status?: string; limit?: number }) => {
    // NEVER executed by the importer. Upstream: api GET /eats/v1/orders (default method).
    const data = await api<{ orders: unknown[] }>('/eats/v1/orders', {
