...
import { z } from 'zod';
import { apiVoid } from '../ubereats-api.js';
...
export const cancelOrder = defineTool({
  ...
  summary: 'cancel my ubereats order',
  icon: 'x-circle',
  group: 'Orders',
  input: z.object({
  ...
  handle: async (params: { order_id: string; reason?: string }) => {
    // NEVER executed by the importer. Upstream: apiVoid DELETE /eats/v1/orders/:id
    // (cancel -> DESTRUCTIVE via the shared verb set; apiVoid {method:'DELETE'} -> apiDelete/destructive).
    await apiVoid(`/eats/v1/orders/${params.order_id}`, { method: 'DELETE', body: { reason: params.reason } });
    return { success: true };
  },
});
