...
export const listRestaurants = defineTool({
  ...
  description:
    'List or search Uber Eats restaurants available for delivery to an address. Optionally filter by a search term (cuisine or restaurant name).',
  summary: 'show me restaurants on ubereats',
  icon: 'utensils',
  group: 'Restaurants',
  input: z.object({
  ...
  handle: async (params: { address?: string; query?: string; limit?: number }) => {
    // NEVER executed by the importer. Upstream: api GET /eats/v1/restaurants (default method).
    const data = await api<{ restaurants: unknown[] }>('/eats/v1/restaurants', {
