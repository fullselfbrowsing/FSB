// Hermetic transport-helper stub for the vendored ubereats metadata slice.
//
// Upstream ubereats-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Uber Eats web app on www.ubereats.com. The importer NEVER executes a handle()
...
// facts (preserved here as comments for auditability): Uber Eats lists restaurants +
// a restaurant's menu + the user's orders via `api` (default GET, reads); PLACING an
// order POSTs the cart (place_order -> the PAYMENT WRITE; the {method:'POST'} literal
// reinforces write on both axes -- a paid order is money moved); CANCELLING an order
// is the DESTRUCTIVE op (cancel_order -> apiVoid {method:'DELETE'} ->
// apiDelete/destructive). www.ubereats.com is SENSITIVE (39-01) -> posture-B re-gates
// the writes, and backing:'dom' keeps place_order DOM-only (-> the payment-op guard
// passes).
...
const inert = (helper: string): never => {
  throw new Error(
    `ubereats-api stub: ${helper} is metadata-only and must never execute at ` +
