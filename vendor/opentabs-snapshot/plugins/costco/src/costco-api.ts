// Hermetic transport-helper stub for the vendored costco metadata slice.
//
// Upstream costco-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Costco web app on www.costco.com. The importer NEVER executes a handle() body
...
// facts (preserved here as comments for auditability): Costco searches the warehouse
// catalog + a single product detail + the user's orders via `api` (default GET,
// reads); PLACING an order POSTs the cart (place_order -> the PAYMENT WRITE; the
// {method:'POST'} literal reinforces write on both axes -- a paid order charges the
// saved card). www.costco.com is SENSITIVE (39-01) -> posture-B re-gates the writes,
// and backing:'dom' keeps place_order DOM-only (NOT API-invocable -> the payment-op
// guard passes).
...
const inert = (helper: string): never => {
  throw new Error(
    `costco-api stub: ${helper} is metadata-only and must never execute at ` +
