...
export const searchProducts = defineTool({
  ...
  description:
    'Search the Costco warehouse catalog by keyword. Optionally filter by category and sort the results.',
  summary: 'search for products on costco',
  icon: 'search',
  group: 'Catalog',
  input: z.object({
    ...
    sort: z.enum(['relevance', 'price_low_to_high', 'price_high_to_low']).optional().describe('Result ordering'),
