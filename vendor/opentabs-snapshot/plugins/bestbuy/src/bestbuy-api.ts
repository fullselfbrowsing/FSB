// Hermetic transport-helper stub for the vendored bestbuy metadata slice.
//
// Upstream bestbuy-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Best Buy web app on www.bestbuy.com. The importer NEVER executes a handle() body
// (Wall 1), but the tool modules reference `api` / `apiVoid` in their (never-run)
...
// facts (preserved here as comments for auditability): Best Buy searches the
// electronics catalog + a single product detail + the user's orders via `api`
// (default GET, reads); PLACING an order POSTs the cart (place_order -> the PAYMENT
// WRITE; the {method:'POST'} literal reinforces write on both axes -- a paid order
// charges the saved card); CANCELLING an order is the DESTRUCTIVE op (cancel_order ->
// apiVoid {method:'DELETE'} -> apiDelete/destructive). www.bestbuy.com is SENSITIVE
// (39-01) -> posture-B re-gates the writes, and backing:'dom' keeps place_order
...
const inert = (helper: string): never => {
  throw new Error(
    `bestbuy-api stub: ${helper} is metadata-only and must never execute at ` +
