// Hermetic transport-helper stub for the vendored stubhub metadata slice.
//
// Upstream stubhub-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// StubHub web app on www.stubhub.com. The importer NEVER executes a handle() body
...
// facts (preserved here as comments for auditability): StubHub searches events + a
// listing's detail + the user's ticket orders via `api` (default GET, reads); BUYING
// tickets POSTs the purchase (buy_tickets -> the PAYMENT op; the {method:'POST'}
// literal classes it write -- 'buy' is NOT a side-effect WRITE_VERB, so the POST is
// REQUIRED; buying resale tickets moves money). www.stubhub.com is SENSITIVE (39-05,
...
const inert = (helper: string): never => {
  throw new Error(
    `stubhub-api stub: ${helper} is metadata-only and must never execute at ` +
