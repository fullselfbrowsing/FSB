// Hermetic transport-helper stub for the vendored walmart metadata slice.
//
// Upstream walmart-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Walmart web app on www.walmart.com. The importer NEVER executes a handle() body
...
// facts (preserved here as comments for auditability): Walmart searches the
// everyday-essentials catalog + a single product detail + the user's orders via
...
// order charges the saved card); CANCELLING an order is the DESTRUCTIVE op
// (cancel_order -> apiVoid {method:'DELETE'} -> apiDelete/destructive).
// www.walmart.com is SENSITIVE (39-01) -> posture-B re-gates the writes, and
...
const inert = (helper: string): never => {
  throw new Error(
    `walmart-api stub: ${helper} is metadata-only and must never execute at ` +
