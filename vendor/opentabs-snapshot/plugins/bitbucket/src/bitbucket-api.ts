// Hermetic transport-helper stub for the vendored bitbucket metadata slice.
//
// Upstream bitbucket-api.ts (SHA 4b170216) imports the real SDK's fetchJSON /
// fetchFromPage / storage helpers and reads document/localStorage against the
// Bitbucket Cloud REST API 2.0 (https://api.bitbucket.org/2.0). The importer NEVER
// executes a handle() body (Wall 1), but the tool modules reference `api` /
...
// facts (preserved here as comments for auditability): Bitbucket is a REST app --
// `api` defaults to GET (reads: list_pull_requests, get_pull_request,
// list_repositories) and is called with {method:'POST'} for create_pull_request;
// `apiVoid` is the 204 mutating helper. The op-name verb (create/list/get) is the
// signal the shared side-effect-class.mjs recognizes (create -> write; list/get ->
// read).
...
const inert = (helper: string): never => {
  throw new Error(
    `bitbucket-api stub: ${helper} is metadata-only and must never execute at ` +
