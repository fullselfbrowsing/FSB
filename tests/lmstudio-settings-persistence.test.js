...
const discoveryAwait = loadSettings.indexOf('const discoveryResult = await setProviderSelection(');
...
assert(saveSettings.includes("modelName: selectedModelName || 'grok-4-1-fast'"), 'hosted fallback remains provider-gated by the LM Studio guard');
