...
- Runtime payload: pin every direct production dependency to the exact version recorded by the committed `mcp/package-lock.json`, list all direct production dependencies in npm `bundleDependencies`, and package the full transitive production closure inside the `fsb-mcp-server@<version>` tarball. A packaged `native-host/runtime-integrity.json` receipt records the committed lockfile digest and every production package path/name/version/integrity; pack/workflow gates reject any receipt, bundled tree, or root dependency declaration that differs from the lock. [VERIFIED: resolved self-contained payload contract]
- Runtime staging: invoke absolute Node plus a validated absolute `npm-cli.js` with argv arrays and `shell:false`; repack only the exact currently installed `fsb-mcp-server@<version>` package and its already bundled dependency tree, then install that absolute tarball with `--offline --ignore-scripts --omit=dev --no-audit --no-fund --package-lock=false` into a tokened temporary owned root. Use a new empty npm cache and a poisoned/unreachable registry/proxy sentinel, validate the pack receipt, tarball digest, package name/version, integrity receipt, complete bundled closure, and expected artifacts before rename, and never resolve or fall back to a registry dependency. [VERIFIED: resolved clean-cache offline materialization contract] [CITED: https://nodejs.org/api/child_process.html]
...
| Recursive dependency copying from an npm cache | npm `bundleDependencies` for every exact-pinned direct production dependency, a lock-derived packaged integrity receipt, and offline installation of the exact current tarball into a staged prefix. | npm dependency layouts may contain nested and linked paths; the tarball must already contain the complete production closure and remain installable with a new empty cache and no reachable registry. [VERIFIED: resolved packaging boundary based on current package execution model] |
...
## Planning Constraints and Residual Assumptions

[VERIFIED: research found no design assumption that should silently become a locked behavior] The table records the resolved Wave 0 choices and the one residual external-interpreter lifecycle assumption that must fail closed:

| Assumption | Risk | Required resolution |
|---|---|---|
| [RESOLVED] The released npm package includes version-matched Windows x64 and arm64 launcher artifacts produced by the Windows build workflow. | A missing artifact makes NATIVE-01 incomplete on Windows. | Plans 63-01-01 and 63-01-03 make source, PE artifacts, checksums, embedded version, workflow dependency, and packed contents blocking; no batch fallback exists. |
...
| [RESOLVED] Exact current-package materialization is self-contained and offline. | A plain tarball with unbundled dependencies would query the registry or fail from a clean cache. | Plans 63-01-02 and 63-05-02 require exact direct pins, npm `bundleDependencies`, a lock-derived integrity receipt, and clean-cache `--offline` materialization with no online fallback. |
| [RESOLVED] `7226` remains the canonical local serve port for this milestone. | A user-selected serve port would not be discoverable without new configuration authority. | Plans 63-02-01 and 63-04-01 pin the existing loopback default; profile/port discovery remains explicitly deferred. |

## Resolved Planning Decisions

All five research questions are resolved by the existing 12-plan decomposition; none remains an executor choice.

| Research question | Exact resolution | Implementing plan/task |
|---|---|---|
| Windows source, build, workflow, and artifact binding | Source is `mcp/native-host/windows/fsb-native-host-bootstrap.c` plus `fsb-native-host-bootstrap-version.rc.in`; `scripts/build-native-host-windows.mjs` builds x64/arm64 PE files at `mcp/native-host/bin/win32-{x64,arm64}/fsb-native-host.exe`; CI and npm publish require embedded package version, architecture metadata, checksums, and packed-artifact identity. No `.bat`, `.cmd`, shell, or SEA fallback exists. | 63-01-01 and 63-01-03 |
| Self-contained stable runtime payload | `mcp/package.json` exact-pins all direct production dependencies and declares them in npm `bundleDependencies`; `mcp/package-lock.json` is the canonical closure/integrity input; packaged `native-host/runtime-integrity.json` binds the lock digest and production closure. Installer repacks only the exact current installed package plus bundled tree and installs the absolute tarball from a new empty cache with `--offline`, poisoned registry/proxy settings, scripts disabled, and no online fallback. macOS and Linux workflow tests must both prove zero registry requests and source/cache removal durability. | 63-01-02 and 63-05-02 |
| Windows registry view | HKCU 32-bit is the sole canonical mutation view because Chrome checks it first; both user 32-bit and 64-bit views are inspected before install/uninstall, any 64-bit value is a shadow/mismatch, and HKLM is never read or mutated. Genuine WOW64/Chrome registry behavior remains pending human UAT. | 63-05-01 and 63-05-03 |
| Health compatibility | Readiness uses dedicated bounded `nativeHostProtocol: 1` plus exact service identity and `serveReady`; patch package-version equality is not the compatibility boundary. The canonical product version remains bounded diagnostic evidence. | 63-02-01 |
| Checking presentation transport | Background emits a separate exact attempt-scoped `{type:'FSB_NATIVE_WAKE_CHECKING',attemptId,intentId}` event for each current intent while native work remains coalesced. Side panel accepts it only for the matching pending intent and permanently fences edited intent state. | 63-08-02 and 63-09-01 |
...
| Residual medium-confidence evidence | Real Windows launcher/registry/Chrome behavior remains unproved locally, while the source/build/package contract itself is fixed by Plans 63-01 and 63-05. [VERIFIED: no current Windows runtime evidence] |
| Human evidence | Entirely deferred and recorded as `human_needed`. [VERIFIED: standing user instruction] |
| Planning resolution gate | Plan 63-01 blocks later NATIVE-01 work on the version-bound Windows executable, exact bundled runtime closure, lock-derived integrity receipt, offline clean-cache materialization proof, and workspace-preserving build wrapper. [VERIFIED: resolved requirement-to-plan mapping] |

## Research Resolution

[VERIFIED: requirements-to-architecture trace and resolved-decision table above] Phase 63 is ready for execution through the existing 12 plans. The five former planning questions are closed by 63-01-01/02/03, 63-02-01, 63-05-01/02/03, 63-08-02, and 63-09-01. Plan 63-01 is the atomic prerequisite for the Windows artifact, self-contained bundled runtime, clean-cache offline proof, and workspace-safe build boundary; no later plan may weaken those gates. No live UAT is required during the phase; automated completion must stay honest about the milestone-end `human_needed` ledger.
