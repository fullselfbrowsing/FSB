...
## Security Remediation Addendum — 2026-07-18 (`F63-SECURITY-01`, `F63-SECURITY-02`)

- Findings `F63-SECURITY-01` and `F63-SECURITY-02` are closed by RED commit `a91f3e29` and GREEN commit `409e638d`.
- This addendum supersedes the `F63-CODE-01` addendum's statement that production uses direct `reg.exe` operations. Production now composes the packaged, exact-architecture Win32 registry helper and does not invoke `reg.exe`, a shell, `PATH` lookup, or an executable derived from inherited `SystemRoot`/`SYSTEMROOT`.
- The adapter validates packaged-helper provenance immediately before every spawn, uses an absolute executable path with `shell:false` and an empty environment, and accepts only the helper's bounded structured response. Successful-but-unparseable output is unavailable, never absent.
- The helper protocol fixes the registry key and exposes only user/64 read plus exact user/32 read/write/delete/empty-key cleanup operations. Production has no arbitrary registry-key or registry-view command surface, and the existing explicit-shadow-absence transaction gates remain authoritative.
- The GREEN gate passed the 15-assertion adversarial helper suite, the 294-assertion runtime transaction section, packaging/workflow/version checks, and source plus compiled boundary verification through the workspace-preserving build wrapper. Genuine Windows compilation and safe execution remain required CI evidence; live registry/browser/native-host UAT remains `human_needed` and unclaimed.
