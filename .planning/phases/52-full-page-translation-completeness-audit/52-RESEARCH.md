...
**How to avoid:** Report the stats-274 section as: "merge status: 15 of 21 keys per locale present as filled `<target>` in the live XLIFF, 6 missing entirely (confirmed via direct diff)" followed by a drift-status line stating whatever count the script's `idDriftFromTemplate` field actually computes for ids renamed-away-from or removed-from the current template since the merge occurred, tracing to template commits `da356602`/`931bc5af` after the JSON's originating commit `46e36899` -- do not hardcode "9" here, this research session found 3 disputed candidate values (7/9/13, see Open Questions #3) and the report should state whatever the script computes, flagged as not yet reconciled with prior research. This is a materially different, more actionable finding than "unverified."
...
function traceStats274(locale, jsonPath, xliffMap, currentTemplateIds) {
  ...
  //   merged.length === 15
  //   missingFromXliff.length === 6 (SHOWCASE_STATS_FSB_SECTION_ARIA, _SECTION_HEADING,
  //     _SECTION_SUB, _CHART_AGENTS_RUNNING, _CHART_AVG_AGENTS, _CHART_AVG_AGENTS_LEGEND --
  //     confirmed absent from the live <target> in all 5 locales, not merely stale)
  //   staleValue.length === 0
  //   idDriftFromTemplate: NOT independently re-verified -- see Open Questions #3. Do not
  //     reuse any specific count from this research session without re-deriving it fresh.
}
```
...
| `translations.stats-274.*.json` merge status treated as "needs investigation" in milestone-level research (SUMMARY.md, ARCHITECTURE.md) | Confirmed 15/21 keys merged per locale (6 missing entirely), with a specific, named 9-id-per-locale (of the 54 total orphans) post-merge drift (see Open Questions #3 -- the script's `idDriftFromTemplate` field may not actually reproduce this 9) | This phase (52) | Phase 53's resync scope is now precisely bounded for the stats-274 portion, not "unknown extent" |
...
3. **What is the correct per-locale count for `idDriftFromTemplate`, and should the reference function's loop scope change?**
   - What we know: the `traceStats274` function's `idDriftFromTemplate` loop (Code Examples section above) scans all 21 `jsonIds`, not just the merged subset — which contradicts its own preceding comment ("which of the merged ids no longer exist in the CURRENT template"). A real run of the code as written produces **13** drifted ids per locale, not the 6 this research session's worked example claimed (that 6 was actually the `missingFromXliff` set, mislabeled).
   - What's unclear: which of three candidate numbers the phase should actually report — **7** (ids that ARE merged into target AND absent from the current EN template — matches the comment's stated intent), **9** (the broader "all target-orphans vs. EN template" figure used elsewhere in this doc — 7 above plus 2 unrelated legacy orphans, `SHOWCASE_STATS_FSB_CHART_PENDING` and `_TILE_AVG_AGENTS_LABEL`, that were never part of the stats-274 JSON at all), or **13** (what the loop as literally written computes).
   - Recommendation: the planner/executor should pick one of these deliberately during Task 2 (most likely 7, to match the loop's own stated intent, with the loop's scope fixed accordingly) rather than trusting any specific count carried over from this research session.
...
| AUDIT-02 | Script correctly reports stats-274 merge status as 15/21 per locale (6 missing) with the id-drift-from-template classification populated (exact count not yet re-verified — see Open Questions #3) | manual-only (smoke, count + spot-check) | Compare report's stats-274 section against confirmed findings in this research (Code Examples section's expected-result comment) | N/A |
...
### Sampling Rate
- **Per task commit:** Run the script manually against the real repo files and spot-check its output against this research's confirmed ground-truth numbers (5 drifted ids, 54 orphans/locale, 15/21 stats-274 merge [6 missing]; id-drift-from-template count not yet re-verified, see Open Questions #3).
...
| Stats-274 JSON keys already merged into live XLIFF `<target>` | 15 of 21 per locale (71%); 6 missing entirely (`SHOWCASE_STATS_FSB_SECTION_ARIA`, `_SECTION_HEADING`, `_SECTION_SUB`, `_CHART_AGENTS_RUNNING`, `_CHART_AVG_AGENTS`, `_CHART_AVG_AGENTS_LEGEND`), identical across all 5 locales | Direct value comparison, JSON value vs. live `<target>` inner text, all 5 locales script-verified |
| Stats-274-related ids present in target XLIFF but absent from current EN template | 9 per locale (a subset of the 54 total orphans) -- this is a target-vs-EN set-diff fact; the script's `idDriftFromTemplate` field is a distinct, disputed computation that may not reproduce this number (see Open Questions #3) | Set-diff, EN's 12 current `SHOWCASE_STATS_FSB_*` ids vs. target's 21 |
