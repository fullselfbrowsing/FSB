...
`approved` and `nyquist_compliant` describe this executable validation design. Every implementation row remains `planned` until its plan executes; the three genuine external scenarios remain `human_needed` regardless of automated results.
...
| 64-09-03 | 09 | 7 | MULTI-01, MULTI-02 | T64-04, T64-10 | Background alone sends adapterId and freezes matching provider/billing metadata at acceptance | `node tests/delegation-routing.test.js && node tests/mcp-bridge-background-dispatch.test.js --section delegation-provider-routing` | ⬜ planned |
| 64-10-01 | 10 | 8 | MULTI-01, MULTI-03 | T64-08, T64-10 | Durable entries accept canonical OpenCode, force unknown billing, and keep result nonterminal | `node tests/delegation-event-store.test.js` | ⬜ planned |
| 64-10-02 | 10 | 8 | MULTI-01, MULTI-03 | T64-08, T64-10 | Controller persists before fanout and silently hydrates concurrent provider identity/exact-once state | `node tests/delegation-controller.test.js` | ⬜ planned |
| 64-10-03 | 10 | 8 | MULTI-01, MULTI-03 | T64-09, T64-10 | Closed adapter-specific drift detail is bounded, identity-matched, and independently throttled | `node tests/agent-protocol-drift-diagnostics.test.js && node tests/mcp-bridge-background-dispatch.test.js --section agent-protocol-drift` | ⬜ planned |
| 64-11-01 | 11 | 8 | MULTI-01, MULTI-02, MULTI-03 | T64-05, T64-10 | Existing Providers model maps safe evidence and exact Not reported copy without selection/billing/native leakage | `node tests/providers-panel-logic.test.js && node tests/mcp-agent-providers-storage.test.js` | ⬜ planned |
| 64-11-02 | 11 | 8 | MULTI-01, MULTI-02, MULTI-03 | T64-10 | Existing second row retains exact DOM/order/copy/focus/a11y with one nonvisual helper script and no CSS/layout branch | `node tests/providers-panel-ui.test.js` | ⬜ planned |
| 64-12-01 | 12 | 9 | MULTI-01, MULTI-02, MULTI-03 | T64-04, T64-10 | Existing consent/lifecycle DOM uses canonical labels, provider-free start, and no topology/replay branch | `node tests/delegation-sidepanel-ui.test.js --section opencode-lifecycle && node tests/providers-panel-ui.test.js --section delegation-trust` | ⬜ planned |
| 64-12-02 | 12 | 9 | MULTI-01, MULTI-02, MULTI-03 | T64-08, T64-10 | Same normalized feed shows Billing not reported and no success before authoritative terminal | `node tests/delegation-sidepanel-ui.test.js && node tests/delegation-event-store.test.js` | ⬜ planned |
| 64-13-01 | 13 | 10 | MULTI-01, MULTI-02, MULTI-03 | T64-10 | Exactly three genuine scenarios remain unchecked human_needed/pending/evidence-empty | `node tests/delegation-phase-contract.test.js --section phase64-uat-ledger` | ⬜ planned |
| 64-13-02 | 13 | 10 | MULTI-01, MULTI-02, MULTI-03 | T64-01–T64-10 | Focused/root/CI/source-security matrix pins the first commit, atomic exposure, transient-secret contract, and workspace preservation exactly once | `node tests/phase64-full-tests-harness.test.js && node scripts/run-phase64-full-tests.mjs` | ⬜ planned |
...
| Contract/profile/composition | `node scripts/run-mcp-build-preserving-workspace.mjs --commands-json '[["node","tests/mcp-agent-provider-contract.test.js"],["node","tests/mcp-opencode-adapter.test.js"],["node","tests/mcp-agent-drift-smoke.test.js"]]'` |
...
**Approval:** approved for execution; implementation rows remain planned and live UAT remains pending.
