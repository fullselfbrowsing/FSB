...
- `.planning/ROADMAP.md` — Phase 53 goal, dependency, success criteria, and boundary from later corpus/truth/view phases.
- `.planning/REQUIREMENTS.md` — Normative HUD-06 fail-quiet and HUD-09 semantic-anchor requirements.
- `.planning/PROJECT.md` — v1.2.0 product framing, host-integrity constraints, Drive/Docs interaction boundary, and approved Chrome-local architecture.
- `.planning/phases/52-on-demand-hud-lifecycle-primitive-shell/52-CONTEXT.md` — Locked invocation, attention, one-shell, focus, stale-work, and teardown decisions that Phase 53 must preserve.

### Architecture, product behavior, and risk
- `.planning/research/ARCHITECTURE.md` — Context-router/anchor-registry separation, semantic anchor descriptor contract, trust boundaries, and dependency-respecting build order.
- `.planning/research/FEATURES.md` — Meaning-attached-to-content product behavior, three Drive/Docs contexts, failure states, and semantic-anchoring rationale.
- `.planning/research/PITFALLS.md` — Drive virtualization/recycled-row failure mode, fail-quiet/rebind guidance, bounded observation, and required anchor regression coverage.
- `.planning/research/SUMMARY.md` — Milestone architecture sequence and the context/anchoring foundation's boundary before permission and truth work.
