...
- [ ] 36-01-PLAN.md — CGEN-01: build-time tsx importer (z.toJSONSchema extraction + recursive Wall-1 forbidden-field pre-scan + classifyGate-before-emit + flat provenance-stamped descriptors); deps hardened to devDeps; phase-wide test/gate registration
- [ ] 36-02-PLAN.md — CGEN-03: the load-bearing resolve() descriptor-only fallback (T3 default / T2 when backing:learn) + no-dead-entry harness + router invoke proof (never RECIPE_NOT_FOUND for a searchable slug)
