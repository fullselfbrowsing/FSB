...
**Requirements:** FINT-19 (owner-chip friendly-label resolver via lookupClientLabel + three-tier resolution), FINT-20 (foreign-owned input lockout via applyInputLockout + defense-in-depth runtime gate), FINT-21 (per-tab conversation state model via sidecar envelope + migration helper + lazy mint per D-17).
...
**Plans:** 5 plans across 5 waves: Wave 0 sidecar module + smoke harness scaffold + CSS baseline; Wave 1 FINT-19 owner-chip lookupClientLabel + three-tier resolution; Wave 2 FINT-20 input lockout helpers + runtime gate + CSS + aria span; Wave 3 FINT-21 per-tab envelope + migration + onActivated swap + onRemoved drop + lazy mint; Wave 4 ceremony closure (REQUIREMENTS.md + LATTICE-PIN.md + v0.10.0-MILESTONE-AUDIT.md + 11-VERIFICATION.md + INV byte-freeze regression smoke).
...
- [ ] `11-03-PLAN.md` (Wave 3) — FINT-21 per-tab conversation state model. `extension/ui/sidepanel.js` boot path refactored: `initConversationId` REPLACED with `initTabConversationStore` consuming Plan 11-00 sidecar migration helper; existing chrome.tabs.onActivated extended with `swapToTabConversation`; new chrome.tabs.onRemoved listener (D-14 drop); NO onDiscarded listener (D-15 preserve); lazy mint per D-17 via `ensureTabConversationForActiveTab(false)` in handleSendMessage + `ensureTabConversationForActiveTab(true)` overwrite in startNewChat; race-free hydration via `_envelopeReadyPromise`. Smoke Parts 5+6 filled at >= 9 PASS. Depends on 11-02 (shared sidepanel.js editing region).
- [ ] `11-04-PLAN.md` (Wave 4) — Ceremony closure. REQUIREMENTS.md FINT-19/20/21 narrative + 3 traceability rows + Total v1 41 → 44 + Last updated bumped; LATTICE-PIN.md Phase 11 row appended with `current_lattice_sha` UNCHANGED per INV-06; v0.10.0-MILESTONE-AUDIT.md status_history phase_11_shipped entry + last_revised bumped + status STAYS in_progress per D-22; new `11-VERIFICATION.md` Human Verification section with 6-sub-assertion UAT-11 procedure joining consolidated UAT-08+09+10+11 single Chrome MV3 reload session; smoke Part 7 INV byte-freeze regression filled (>= 4 PASS covering setTimeout count + iterator patterns + Phase 11 token awk-scan + LATTICE-PIN SHA byte-freeze). Smoke final >= 37 PASS / 0 FAIL. Depends on 11-03.
