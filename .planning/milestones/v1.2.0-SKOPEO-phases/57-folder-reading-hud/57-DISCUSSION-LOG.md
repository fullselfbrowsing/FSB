...
| Assumption | Confidence | Evidence |
|------------|------------|----------|
| Keep notice deadline, renewal, termination, and expiration explicitly typed with the consequence of inaction separate. Render gaps only from authoritative evidence and reserve first-class neutral slots for policy/memo/notification statuses until Phase 58 or 59 provides the authority to claim a required memo, missing policy obligation, or notification failure. | Likely | `.planning/milestones/v1.2.0-SKOPEO-REQUIREMENTS.md`; `extension/utils/skopeo-corpus-schema.js`; `extension/utils/skopeo-graph-schema.js`; `extension/utils/skopeo-truth-schema.js`; Phase 58–59 roadmap boundaries |
