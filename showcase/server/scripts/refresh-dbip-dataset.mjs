...
async function main() {
  ...
  for (const r of rows) {
    ...
    if (last && last[2] === r[2] && last[3] === r[3] && r[0] <= last[1] + 1) {
      // Same label + adjacent/overlapping -> extend the end (max handles a
      // fully-contained range without shrinking the span). Different
      // subdivisions within one country stay distinct (US/California vs US/Nevada).
      if (r[1] > last[1]) last[1] = r[1];
    } else {
      merged.push([r[0], r[1], r[2], r[3]]); // copy so last[1] is safe to mutate
    }
  }
