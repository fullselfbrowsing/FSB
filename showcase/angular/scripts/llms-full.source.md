...
- All detected hosts: `npx -y fsb-mcp-server@latest install --all`

Append `--dry-run` to preview without writing, sanity-check with `npx -y fsb-mcp-server@latest doctor`, and restart the client so the new MCP server appears. The extension pairs with the local bridge on `ws://localhost:7225`; optional Streamable HTTP mode exposes `http://127.0.0.1:7226/mcp`.
