...
- [Agents](https://full-selfbrowsing.com/agents): OpenClaw skill onboarding, ClawHub one-click install (`https://clawhub.ai/lakshmanturlapati/full-selfbrowsing`), FSB MCP power story (66 MCP tools, multi-agent identity, vault boundary, real Chrome session), 3-step install, and grounded use cases. Autopilot (`run_task`) only fires on explicit user delegation; manual tool calls are default.
...
- [GitHub repository](https://github.com/fullselfbrowsing/FSB): BSL 1.1 source code, install instructions, issue tracker.
